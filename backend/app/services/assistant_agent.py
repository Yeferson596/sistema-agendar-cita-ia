...
def run_agent_query(db: Session, question: str, conversation_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if settings.ai_provider == "openai":
        selected_provider = "openai" if use_openai else None
    elif settings.ai_provider == "roq":
        selected_provider = "roq" if use_roq else None
    elif settings.ai_provider == "auto":
        selected_provider = "openai" if use_openai else "roq" if use_roq else None
    else:
        selected_provider = None

    if selected_provider == "roq":
    ...
    if selected_provider != "openai" or not LANGCHAIN_AVAILABLE:
        response = answer_question(question)
        return {
            "question": question,
            "answer": response["answer"],
