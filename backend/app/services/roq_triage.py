...
def _generate_roq_triage(description: str) -> Dict[str, Any]:
    if not GROQ_AVAILABLE:
        logger.debug("Groq library not installed, skipping")
        return None
    
    if not settings.roq_api_key:
        logger.debug("ROQ_API_KEY not configured, skipping")
        return None
    ...
    try:
        ...
        system_prompt = """
ERES UN MÉDICO ESPECIALISTA EN TRIAJE MÉDICO PROFESIONAL.
FUNCIONAS EXCLUSIVAMENTE PARA CLASIFICAR SÍNTOMAS DE PACIENTES.

✅ NORMAS OBLIGATORIAS:
1. SELECCIONA SOLAMENTE UNA ESPECIALIDAD DE LA LISTA:
Odontología, Pediatría, Cardiología, Traumatología, Dermatología, Gastroenterología, Otorrinolaringología, Oftalmología, Neumonología, Neurología, Endocrinología, Nefrología, Medicina General

2. NIVEL DE URGENCIA SOLAMENTE UNO:
high -> urgente, emergencias graves
medium -> necesita atención pero no emergencia
low -> consulta leve, control o rutina

3. DEVUELVE SOLAMENTE JSON VÁLIDO, NADA MAS. NINGÚN TEXTO ADICIONAL.
FORMATO EXACTO:
{
  "specialty": "Nombre especialidad",
  "urgency": "low|medium|high",
  "reasoning": "Explicación breve en español de max 2 frases"
}
        """
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Paciente: {description}"}
            ],
            temperature=0.1,
            max_tokens=180,
        ...
        return {
            "specialty": parsed.specialty.strip(),
            "urgency": _normalize_urgency(parsed.urgency),
            "reasoning": parsed.reasoning.strip()
        }
        
    except Exception as e:
        logger.warning("✅ ROQ/GROQ falló: %s", str(e))
        return None
...
def perform_triage(description: str) -> Dict[str, Any]:
    ...
    if settings.ai_provider == "openai":
        if use_openai:
            try:
                ...
                logger.warning("OpenAI triaje falló: %s", e)
    elif settings.ai_provider == "roq":
        if use_roq:
            logger.info("✅ Usando ROQ/GROQ para triaje")
            result = _generate_roq_triage(description)
            if result:
                return result
    else:  # auto o valor inválido
        if use_openai:
            try:
                return _generate_openai_triage(description)
            except Exception as e:
    ...
    logger.info("Usando triaje fallback manual")
