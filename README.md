...
1. Base de datos: copia `backend/.env.example` a `backend/.env` y ajusta `DATABASE_URL`, `JWT_SECRET`, `ADMIN_EMAILS` (correos con rol admin), `OPENAI_API_KEY` (recomendado para el agente) y `GOOGLE_CLIENT_ID` (opcional, login Google en backend).
...
- `OPENAI_API_KEY` es necesario para el agente LangChain, la ejecución de herramientas y la memoria semántica en PGVector.
- El agente de `POST /assistant` está construido con LangChain y usa OpenAI cuando `AI_PROVIDER=auto` y `OPENAI_API_KEY` está configurada.
- `ROQ_API_KEY` es opcional y sólo se usa como respaldo cuando `AI_PROVIDER=roq` o cuando `AI_PROVIDER=auto` y OpenAI no está disponible.
